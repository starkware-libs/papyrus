...
async fn signed_headers_basic_flow() {
    ...
    let block_hashes_and_signatures =
        create_block_hashes_and_signatures((NUM_QUERIES * QUERY_SIZE).try_into().unwrap());
    ...
    let parse_queries_future = async move {
        for query_index in 0..NUM_QUERIES {
            let start_block_number = query_index * QUERY_SIZE;
            let end_block_number = (query_index + 1) * QUERY_SIZE;
...
async fn sync_sends_new_query_if_it_got_partial_responses() {
    ...
    assert!(usize::from(NUM_ACTUAL_RESPONSES) < QUERY_SIZE);
    ...
    let parse_queries_future = async move {
        ...
        let query = timeout(Duration::from_secs(1), query_receiver.next()).await.unwrap().unwrap();
