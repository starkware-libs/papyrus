...
const MAX_FEE: u128 = 1000000 * 100000000000; // 1000000 * min_gas_price.
...
fn estimate_fees_scenario(txs: Vec<ExecutableTransactionInput>) {
    let ((storage_reader, storage_writer), _temp_dir) = get_test_storage();
    prepare_storage(storage_writer);

    let chain_id = ChainId(CHAIN_ID.to_string());

    let fee_estimations = estimate_fee(
        txs,
        &chain_id,
        &storage_reader.begin_ro_txn().unwrap(),
        StateNumber::right_after_block(BlockNumber(0)),
    ...
    for fee_estimation in fee_estimations {
        assert_ne!(fee_estimation, Fee(0));
    }
}

// TODO(yair): Find the expected fees for the following scenarios.
#[test]
