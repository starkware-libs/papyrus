...
pub fn estimate_fee(
    ...
    Ok(txs_execution_info
        .into_iter()
        .enumerate()
        .map(|(index, (tx_execution_info, _induced_state_dif, price_unit))| {
...
fn execute_transactions(
    ...
    {
        let price_unit = match tx {
            ExecutableTransactionInput::Invoke(InvokeTransaction::V3(_), _)
            | ExecutableTransactionInput::DeclareV3(_, _, _)
            | ExecutableTransactionInput::DeployAccount(DeployAccountTransaction::V3(_), _) => {
                PriceUnit::Fri
            }
            _ => PriceUnit::Wei,
        };
