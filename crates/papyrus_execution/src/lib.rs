...
const DEFAULT_CONFIG_FILE: &str = "default_config.json";
...
impl ExecutionConfig {
    ...
    fn get_config_file(&self) -> File {
        // TODO(Omri): Find a better way to get the config file path.
        let home_dir = match std::env::var("CARGO_MANIFEST_DIR") {
            Ok(s) => format!("{}/../..", s),
            _ => "".to_string(),
        };
        let file_full_name = Path::new(&home_dir)
            .join("config/execution_config")
            .join(self.config_file_name.as_str());
...
#[derive(Clone, Serialize, Deserialize, Debug, PartialEq)]
#[allow(missing_docs)]
/// Holds a mapping from the block number, indicating the block a version was updated,
/// to the execution configuration corresponding to that version.
...
impl Default for ExecutionConfigByBlock {
    fn default() -> Self {
        ...
        let default_config_file = execution_config.get_config_file();
...
impl TryFrom<ExecutionConfig> for ExecutionConfigByBlock {
    ...
    fn try_from(execution_config: ExecutionConfig) -> Result<Self, Self::Error> {
        let config_file = execution_config.get_config_file();
