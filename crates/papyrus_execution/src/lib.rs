    ...
    Ok(txs_execution_info
        ...
        .map(|(index, tx_execution_output)| {
            ...
            if let Some(revert_reason) = tx_execution_output.execution_info.revert_error {
                Ok::<_, ExecutionError>(Err(RevertedTransaction { index, revert_reason }))
            } else {
                Ok(Ok(tx_execution_output_to_fee_estimation(&tx_execution_output, &block_context)?))
            }
        })
        .collect::<Result<FeeEstimationResult, _>>()?)
}
