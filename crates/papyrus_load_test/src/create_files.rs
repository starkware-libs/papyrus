...
use crate::{get_random_block_number, path_to_resources};

const BLOCK_NUMBER_NUMS: u32 = 5;
...
pub async fn create_files(node_address: &str) {
    ...
    block_number(BLOCK_NUMBER_NUMS);
}

// Creates the block_number.txt file. Write to the file block_num block numbers.
fn block_number(block_number_nums: u32) {
    let mut to_write = String::new();
    for _ in 0..block_number_nums {
    ...
    let mut file = File::create(path_to_resources("block_number.txt"))
...
async fn last_block_number(node_address: SocketAddr) {
    ...
    let mut file = File::create(path_to_resources("last_block_number.txt")).unwrap();
