...
pub async fn post_jsonrpc_request(user: &mut GooseUser, request: &jsonVal) -> PostResult {
    ...
    // The purpose of this struct and the line after it is to report on failed requests.
    // The response.json::<TransactionReceiptResponse> deserialize the body of response to
    // TransactionReceiptResponse. If the response is an error, the result field isn't found in
