...
// block_number
pub async fn block_number<T: DeserializeOwned>(user: &mut GooseUser) -> MethodResult<T> {
    post_jsonrpc_request(user, "starknet_blockNumber", json!([])).await
}

// block_hash_and_number
pub async fn block_hash_and_number<T: DeserializeOwned>(user: &mut GooseUser) -> MethodResult<T> {
...
pub async fn get_storage_at_by_number<T: DeserializeOwned>(
    ...
    post_jsonrpc_request(
        ...
        json!([contract_address, key, { "block_number": block_number }]),
    )
    .await
}
...
pub async fn get_storage_at_by_hash<T: DeserializeOwned>(
    ...
    post_jsonrpc_request(
        user,
        "starknet_getStorageAt",
        json!([contract_address, key, { "block_hash": block_hash }]),
    )
    .await
}
...
pub async fn get_transaction_by_hash<T: DeserializeOwned>(
    ...
    post_jsonrpc_request(user, "starknet_getTransactionByHash", json!([transaction_hash])).await
}
...
pub async fn get_transaction_by_block_id_and_index_by_number<T: DeserializeOwned>(
    ...
    post_jsonrpc_request(
        ...
        json!([{ "block_number": block_number }, index]),
    )
    .await
}
...
pub async fn get_transaction_by_block_id_and_index_by_hash<T: DeserializeOwned>(
    ...
    post_jsonrpc_request(
        user,
        "starknet_getTransactionByBlockIdAndIndex",
        json!([{ "block_hash": block_hash }, index]),
    )
    .await
}
...
pub async fn get_transaction_receipt<T: DeserializeOwned>(
    ...
    post_jsonrpc_request(user, "starknet_getTransactionReceipt", json!([transaction_hash])).await
}
...
pub async fn get_class_by_number<T: DeserializeOwned>(
    ...
    post_jsonrpc_request(
        user,
        "starknet_getClass",
        json!([{ "block_number": block_number }, class_hash]),
    )
    .await
}
pub async fn get_class_by_hash<T: DeserializeOwned>(
    ...
    post_jsonrpc_request(
        user,
        "starknet_getClass",
        json!([{ "block_hash": block_hash }, class_hash]),
    )
    .await
}
