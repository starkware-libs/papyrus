...
async fn run_threads(config: Config) -> anyhow::Result<()> {
    ...
    let (_, server_handle) =
        run_server(&config.gateway, &shared_sync_status, storage_reader.clone()).await?;
    let server_handle_future = tokio::spawn(server_handle.stopped());

    // Sync task.
    let sync_future = run_sync(config, shared_sync_status, storage_reader.clone(), storage_writer);
