...
/// Function to generate a set of tuples containing transaction type and version
fn get_all_transaction_types() -> HashSet<TransactionInfo> {
    ...
    enum_values.insert(TransactionInfo {
        ...
        transaction_version: "0x0".to_string(),
    });
    enum_values.insert(TransactionInfo {
        transaction_type: "DECLARE".to_string(),
        transaction_version: "0x1".to_string(),
    });
    enum_values.insert(TransactionInfo {
        transaction_type: "DECLARE".to_string(),
        transaction_version: "0x2".to_string(),
    });
    enum_values.insert(TransactionInfo {
        transaction_type: "DECLARE".to_string(),
        transaction_version: "0x3".to_string(),
    });
    enum_values.insert(TransactionInfo {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut block_number: u64 = if deprecated {
        ...
        get_current_block_number_with_rpc(&client, node_url.clone()).await?
    };

    while block_number > 0 && !transaction_types.is_empty() {
        println!("Processing block number: {}", block_number);
        let block_transactions_from_rpc =
            get_block_transactions_with_rpc(&client, node_url.clone(), block_number).await?;
        let block_transactions = block_transactions_from_rpc["result"]["transactions"]
            .as_array()
...
fn construct_transaction_from_value(
    ...
    println!("Constructing transaction from type: {} {}", transaction_type, transaction_version);
    let transaction_map =
    ...
    match transaction_type {
        "DECLARE" => match transaction_version {
        ...
        "INVOKE" => match transaction_version {
        ...
        "DEPLOY_ACCOUNT" => match transaction_version {
...
async fn get_block_transactions_with_rpc(
    client: &Client,
    node_url: String,
    block_number: u64,
...
async fn get_current_block_number_with_rpc(
    client: &Client,
    node_url: String,
