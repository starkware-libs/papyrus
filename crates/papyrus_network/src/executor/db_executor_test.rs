...
use crate::streamed_data_protocol::{InboundSessionId, SessionId};
...
async fn receive_queries_from_multiple_senders() {
    ...
    let block_query1 = BlockQuery {
        session_id: SessionId::Inbound(InboundSessionId { value: 1 }),
        ..Default::default()
    };
    let mut query_sender2 = executor.get_query_sender();
    let block_query2 = BlockQuery {
        session_id: SessionId::Inbound(InboundSessionId { value: 2 }),
        ..Default::default()
    };
