...
fn both_protocols_have_same_info() {
    let outbound_protocol = OutboundProtocol::<BlockHeadersRequest> { query: Default::default() };
    let inbound_protocol = InboundProtocol::<BlockHeadersRequest>::new();
    assert_eq!(
...
async fn positive_flow() {
    ...
    // messages is fixed.
    let query = BlockHeadersRequest {
        iteration: Some(protobuf::Iteration { limit: 10, ..Default::default() }),
    };
