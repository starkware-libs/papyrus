...
trait BehaviourTrait {
    ...
    fn handle_received_data(
        ...
        for message in data.part {
            ...
            match message {
                ...
                protobuf::block_headers_response_part::HeaderMessage::Fin(protobuf::Fin {
                    ...
                    // TODO: try to come up with a solution to mark successful session when multiple
                    // parts and no error fin is received.
        ...
        if !received_data.is_empty() {
            ...
            Some(Event::ReceivedData { data: received_data, outbound_session_id })
        } else if got_header {
            None
        } else {
