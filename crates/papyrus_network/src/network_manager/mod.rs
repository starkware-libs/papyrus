...
impl<SwarmT: SwarmTrait> GenericNetworkManager<SwarmT> {
    ...
    fn handle_gossipsub_behaviour_event(&mut self, event: gossipsub_impl::ExternalEvent) {
        match event {
            gossipsub_impl::ExternalEvent::Received { originated_peer_id, message, topic_hash } => {
                ...
                let report_callback = Box::new(|| {
                    let _ = report_callback.send(());
                });
                self.consume_report_receiver(originated_peer_id, report_receiver);
    ...
    fn handle_local_sqmr_query(
        ...
        match self.swarm.send_query(query, PeerId::random(), protocol.clone()) {
            Ok(outbound_session_id) => {
                ...
                {
                    Ok(peer_id) => {
                        self.consume_report_receiver(peer_id, report_receiver);
                    }
                    Err(e) => {
                        error!(
                            "Failed to get peer id from session id. Session id: \
    ...
    fn consume_report_receiver(&self, peer_id: PeerId, report_receiver: oneshot::Receiver<()>) {
...
pub type SubscriberReceiver<T> = Map<Receiver<Bytes>, ReceivedMessagesConverterFn<T>>;
