...
pub struct GenericNetworkManager<SwarmT: SwarmTrait> {
    swarm: SwarmT,
    protocol_to_buffer_size: HashMap<Protocol, usize>,
...
impl<SwarmT: SwarmTrait> GenericNetworkManager<SwarmT> {
    ...
    pub(crate) fn generic_new(swarm: SwarmT) -> Self {
        ...
        Self {
            swarm,
            protocol_to_buffer_size: HashMap::new(),
    ...
    where
        ...
        Query: TryFrom<Bytes>,
    {
        if let Some(old_buffer_size) = self.protocol_to_buffer_size.insert(protocol, buffer_size) {
            debug!(
                "Protocol '{}' has already been registered as a server. Overwriting buffer size \
                 from {} to {}.",
                protocol, old_buffer_size, buffer_size
            );
        }
        ...
        match event {
            sqmr::behaviour::ExternalEvent::NewInboundSession {
                ...
                let (response_sender, response_receiver) = futures::channel::mpsc::channel(
                    *self
                        .protocol_to_buffer_size
                        .get(&protocol)
                        .expect("Received a query for a protocol that has not been registered."),
                );
