...
impl TryFrom<protobuf::ContractDiff> for ThinStateDiff {
    ...
    fn try_from(value: protobuf::ContractDiff) -> Result<Self, Self::Error> {
        ...
        // The p2p specs don't separate replaced classes from deployed contracts. In RPC v0.8 the
        // node will stop separating them as well. Until then nodes syncing from P2P won't be able
        // to separate replaced classes from deployed contracts correctly
        ...
        Ok(ThinStateDiff { deployed_contracts, storage_diffs, nonces, ..Default::default() })
    }
}
    ...
    fn try_from(entry: protobuf::ContractStoredValue) -> Result<Self, Self::Error> {
        ...
        let key = StorageKey(key_felt.try_into().map_err(|_| {
            ProtobufConversionError::OutOfRangeValue {
                ...
                // ContractStoredValue::key should be changed.
