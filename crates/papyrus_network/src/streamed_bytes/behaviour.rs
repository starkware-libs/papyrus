...
pub struct Behaviour {
    ...
    queries_pending_peer_assignment: HashMap<OutboundSessionId, (Bytes, StreamProtocol)>,
}
...
impl Behaviour {
    pub fn new(config: Config) -> Self {
        Self {
            ...
            queries_pending_peer_assignment: Default::default(),
        }
    }
    ...
    pub fn start_query(
        ...
        self.queries_pending_peer_assignment.insert(outbound_session_id, (query, protocol_name));
        ...
        match event {
            FromOtherBehaviour::SessionAssigned { outbound_session_id, peer_id, connection_id } => {
                ...
                let Some((query, protocol_name)) =
                    self.queries_pending_peer_assignment.remove(&outbound_session_id)
                else {
                    error!(
                        "Outbound session assigned peer but it isn't in \
                         queries_pending_peer_assignment. Not running query."
                    );
                    return;
                };
