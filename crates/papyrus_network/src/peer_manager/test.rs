...
async fn peer_block_realeased_after_timeout() {
    ...
    peer.set_timeout_duration(Duration::seconds(1));
...
fn wrap_around_in_peer_assignment() {
    ...
    peer1.expect_is_blocked().times(1).return_const(true);
    ...
    peer2.expect_is_blocked().times(1).return_const(false);
    peer2.expect_multiaddr().times(1).return_const(Multiaddr::empty());
