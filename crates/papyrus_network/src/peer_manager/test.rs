...
fn peer_assignment_round_robin() {
    ...
    match res1.unwrap().0 {
        peer_id if peer_id == peer1.get_id() => {
            assert_eq!(res2.unwrap().0, peer2.get_id());
            assert_eq!(res3.unwrap().0, peer1.get_id());
        }
        peer_id if peer_id == peer2.get_id() => {
            assert_eq!(res2.unwrap().0, peer1.get_id());
            assert_eq!(res3.unwrap().0, peer2.get_id());
        }
...
fn report_query_calls_update_reputation() {
    ...
    peer.expect_get_multiaddr().times(1).return_const(Multiaddr::empty());
...
fn create_mock_peer(
    ...
    peer.expect_get_id().return_const(peer_id);
