...
use crate::messages::proto::p2p::proto::GetBlocks;
...
async fn positive_flow() {
    ...
    let request: GetBlocks = Default::default();
    let (outbound_protocol, mut outbound_responses_relay_receiver) =
        OutboundProtocol::new(request.clone());
    let (inbound_protocol, (inbound_request_relay_receiver, inbound_responses_relay_sender)) =
        InboundProtocol::new();
    ...
    tokio::join!(
        ...
        async move {
            let blocks_request = inbound_request_relay_receiver.await.unwrap();
            assert_eq!(blocks_request, request);
            for expected_response in hardcoded_responses() {
                let msg =
                    if expected_response.is_fin() { None } else { Some(expected_response.clone()) };
                match inbound_responses_relay_sender.unbounded_send(msg) {
                    Ok(_) => {
                        let result = outbound_responses_relay_receiver.next().await;
...
async fn inbound_sends_invalid_response() {
    ...
    tokio::join!(
        ...
        async move { assert!(outbound_responses_relay_receiver.next().await.is_none()) }
    );
}
...
async fn outbound_receiver_closed() {
    ...
    let (outbound_protocol, mut outbound_responses_relay_receiver) =
        OutboundProtocol::new(Default::default());
    let (inbound_protocol, _) = InboundProtocol::new();
    outbound_responses_relay_receiver.close();
...
async fn response_relay_stops_unexpectedly() {
    ...
    let (outbound_protocol, mut outbound_responses_relay_receiver) =
        OutboundProtocol::new(Default::default());
    let (inbound_protocol, (inbound_request_relay_receiver, mut inbound_responses_relay_sender)) =
        InboundProtocol::new();
    ...
    tokio::join!(
        ...
        async move {
            let _blocks_request = inbound_request_relay_receiver.await.unwrap();
            let responses = hardcoded_responses();
            match inbound_responses_relay_sender.unbounded_send(Some(responses[0].clone())) {
                Ok(_) => {
                    let result = outbound_responses_relay_receiver.next().await;
            ...
            let _res = inbound_responses_relay_sender.close().await;
            assert!(outbound_responses_relay_receiver.try_next().is_err());
        }
    );
}
