...
pub const PROTOCOL_NAME: &str = "/example/1.0.0";

#[test]
fn outbound_protocol_info() {
    let outbound_protocol =
        OutboundProtocol { query: Default::default(), protocol_name: PROTOCOL_NAME.into() };
    assert_eq!(outbound_protocol.protocol_info().collect::<Vec<_>>(), vec![PROTOCOL_NAME]);
}

#[test]
fn inbound_protocol_info() {
    let protocol_names = vec![PROTOCOL_NAME.into(), "/example/2.0.0".into()];
...
async fn positive_flow() {
    ...
    let outbound_protocol =
        OutboundProtocol { query: query.clone(), protocol_name: PROTOCOL_NAME.into() };
    let inbound_protocol = InboundProtocol::new(vec![PROTOCOL_NAME.into()]);
    ...
    tokio::join!(
        ...
        async move {
            let mut stream = outbound_protocol
                .upgrade_outbound(outbound_stream, PROTOCOL_NAME.into())
                .await
                .unwrap();
...
async fn inbound_dropped() {
    let (inbound_stream, outbound_stream, _) = get_connected_streams().await;
    let outbound_protocol =
        OutboundProtocol { query: vec![0u8], protocol_name: PROTOCOL_NAME.into() };
    ...
    assert!(
        outbound_protocol.upgrade_outbound(outbound_stream, PROTOCOL_NAME.into()).await.is_err()
    );
}
