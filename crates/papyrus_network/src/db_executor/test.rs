    ...
    tokio::select! {
        ...
        res = receiver.collect::<Vec<_>>() => {
            ...
            for (i, data) in res.iter().enumerate() {
                assert_matches!(data, super::Data::BlockHeaderAndSignature { header: BlockHeader { block_number: BlockNumber(block_number), .. }, ..} if block_number == &(i as u64 + 1));
            }
        }
    }
}
...
async fn header_db_executor_query_of_missing_block() {
    ...
    const NUM_BLOCKS_DELTA: u64 = 5;
    // register a query.
    let (sender, receiver) = futures::channel::mpsc::channel(BUFFER_SIZE);
    let query = BlockQuery {
        start_block: BlockNumber(NUM_OF_BLOCKS - NUM_BLOCKS_DELTA),
        direction: Direction::Forward,
        limit: NUM_OF_BLOCKS,
        step: 1,
    };
    ...
    tokio::select! {
        ...
        res = receiver.collect::<Vec<_>>() => {
            assert_eq!(res.len(), (NUM_BLOCKS_DELTA + 1) as usize);
        }
    }
}
