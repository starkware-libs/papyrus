        ...
        B: BufMut,
    {
        match self {
            ...
            Data::StateDiff { state_diff } => {
                ...
                let mut iter = state_diffs_response_vec.0.iter().map(|data| {
                    let mut buf = vec![];
                    data.encode(&mut buf).map_err(|_| DataEncodingError).map(|_| buf)
                });
                if iter.any(|res: Result<Vec<u8>, DataEncodingError>| res.is_err()) {
                    return Err(DataEncodingError);
                }
                let res_bytes = iter.fold(vec![], |mut acc, res| {
                    acc.extend(res.expect(
                        "Failed to encode state diff - this should never happen since we checked \
                         the results already.",
                    ));
                    acc
                });
                res_bytes.iter().for_each(|byte| buf.put_u8(*byte));
                Ok(())
            }
