...
fn set_initial_version_if_needed(mut writer: StorageWriter) -> StorageResult<StorageWriter> {
    ...
    if current_storage_version_state.is_none() {
        ...
        // If in FullArchive mode, also set the block version.
...
fn verify_storage_version(reader: StorageReader, storage_scope: StorageScope) -> StorageResult<()> {
    ...
    match storage_scope {
        // In state-only mode, the storage should not have a block version.
        StorageScope::StateOnly => {
            if current_storage_version_blocks.is_some() {
                return Err(StorageError::StorageVersionInconcistency(
        ...
        // In full-archive mode, the storage should have a block version and the version should
        // match the crate version.
        StorageScope::FullArchive => {
            ...
            let current_storage_version_blocks = current_storage_version_blocks
                .expect("Storage should have a block version for scope FullArchive");
