...
pub enum StorageError {
    ...
    #[error(
        "State diff redefined a nonce {nonce:?} for contract {contract_address:?} at ommer block \
         {block_hash:?}."
    )]
    OmmerNonceReWrite { nonce: Nonce, block_hash: BlockHash, contract_address: ContractAddress },
    #[error(transparent)]
...
// Keeping serialization of (StateDiff, Vec<DeclaredContract>)
// since the storage serde doesn't handle complex types.
#[derive(Serialize, Deserialize)]
