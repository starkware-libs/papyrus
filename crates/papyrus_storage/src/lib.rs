...
impl<'env, Mode: TransactionKind> StorageTxn<'env, Mode> {
    pub(crate) fn open_table<K: StorageSerde + Debug, V: StorageSerde + Debug, T>(
        &self,
...
struct_field_names! {
    struct Tables {
        ...
        storage_version: TableIdentifier<String, Version, SimpleTable>
    }
}
...
pub(crate) type MarkersTable<'env> = TableHandle<'env, MarkerKind, BlockNumber, SimpleTable>;
...
fn open_storage_files(
    ...
    db_reader: DbReader,
    file_offsets_table: &TableIdentifier<OffsetKind, usize, SimpleTable>,
