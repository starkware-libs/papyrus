use super::Table;
use crate::db::{DbCursor, DbCursorTrait, DbReader, DbWriter, TableHandle, TableIdentifier, RO};

pub(crate) fn table_test<T>(
    table_id: TableIdentifier<(u32, u32), u32, T>,
    reader: &DbReader,
    writer: &mut DbWriter,
) where
    for<'env> TableHandle<'env, (u32, u32), u32, T>: Table<'env, Key = (u32, u32), Value = u32>,
{
...
pub(crate) fn table_cursor_test<T>(
    table_id: TableIdentifier<(u32, u32), u32, T>,
    reader: &DbReader,
    writer: &mut DbWriter,
) where
    for<'env> TableHandle<'env, (u32, u32), u32, T>:
        Table<'env, Key = (u32, u32), Value = u32, TableType = T>,
    for<'txn> DbCursor<'txn, RO, (u32, u32), u32, T>: DbCursorTrait<Key = (u32, u32), Value = u32>,
{
