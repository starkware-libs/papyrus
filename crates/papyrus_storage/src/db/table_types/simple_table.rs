...
impl<'env, K: KeyTrait + Debug, V: ValueSerde + Debug>
     TableHandle<'env, K, V, SimpleTable>{
        // Append key value pair to the end of the table. The key must be the lase in the table, otherwise an error will be returned.
        pub(crate) fn append(
            &'env self,
            txn: &DbTransaction<'env, RW>,
            key: &K,
            ...
            let bin_key = key.serialize()?;
            txn.txn.put(&self.database, bin_key, data, WriteFlags::APPEND)?;
            Ok(())
        }
    }
