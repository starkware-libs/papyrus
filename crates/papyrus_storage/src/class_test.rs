...
#[test]
fn append_classes_class_marker_surpasses_state_marker() {
    let ((_reader, mut writer), _temp_dir) = get_test_storage();

    let Err(err) =
        writer.begin_rw_txn().unwrap().append_classes(BlockNumber(0), &Vec::new(), &Vec::new())
    else {
        panic!("Unexpected Ok.");
    };
    ...
    assert_matches!(
        err,
        StorageError::MarkersUnorderingAttempt { surpassing_marker_kind, surpassed_marker_kind } if
...
#[test]
fn append_classes_class_and_state_diff_mismatch() {
    let ((_reader, mut writer), _temp_dir) = get_test_storage();
    ...
    let Err(err) = writer
        ...
        .append_classes(
            BlockNumber(0),
            &[(ClassHash::default(), &ContractClass::default())],
            &[(ClassHash::default(), &DeprecatedContractClass::default())],
        )
    else {
        panic!("Unexpected Ok.");
    };
    ...
    assert_matches!(
        err,
        StorageError::ClassesStateDiffMismatch { block_number } if block_number.0 == 0
    );
}
