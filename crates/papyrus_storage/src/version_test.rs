...
fn open_storage_full_archive_different_state_major_versions() {
    ...
    let different_state_major_version = get_different_major_version(STORAGE_VERSION_STATE.major);

    // TODO(dvir): consider extract this to a helper function.
    let wtxn = writer.begin_rw_txn().unwrap();
    let version_table = wtxn.open_table(&wtxn.tables.storage_version).unwrap();
    version_table
        .upsert(&wtxn.txn, &VERSION_STATE_KEY.to_string(), &different_state_major_version)
        .unwrap();
    wtxn.commit().unwrap();
    drop(reader);
    drop(writer);
...
fn open_storage_full_archive_different_blocks_major_versions() {
    ...
    let different_blocks_major_version = get_different_major_version(STORAGE_VERSION_BLOCKS.major);

    let wtxn = writer.begin_rw_txn().unwrap();
    let version_table = wtxn.open_table(&wtxn.tables.storage_version).unwrap();
    version_table
        .upsert(&wtxn.txn, &VERSION_BLOCKS_KEY.to_string(), &different_blocks_major_version)
        .unwrap();
    wtxn.commit().unwrap();
    drop(reader);
    drop(writer);
...
fn get_different_major_version(current_major_version: u32) -> Version {
    let mut rng = rand::thread_rng();
    let minor = rng.gen();
    let mut major = rng.gen();
