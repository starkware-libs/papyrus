...
pub trait BaseLayerStorageReader {
    ...
    fn get_base_layer_tip_marker(&self) -> StorageResult<BlockNumber>;
}
    ...
    Self: Sized,
{
    ...
    fn update_base_layer_tip_marker(self, block_number: &BlockNumber) -> StorageResult<Self>;
}

impl<'env, Mode: TransactionKind> BaseLayerStorageReader for StorageTxn<'env, Mode> {
    fn get_base_layer_tip_marker(&self) -> StorageResult<BlockNumber> {
        let markers_table = self.txn.open_table(&self.tables.markers)?;
        Ok(markers_table.get(&self.txn, &MarkerKind::BaseLayerTip)?.unwrap_or_default())
    }
}
...
impl<'env> BaseLayerStorageWriter for StorageTxn<'env, RW> {
    fn update_base_layer_tip_marker(self, block_number: &BlockNumber) -> StorageResult<Self> {
        let markers_table = self.txn.open_table(&self.tables.markers)?;
        markers_table.upsert(&self.txn, &MarkerKind::BaseLayerTip, block_number)?;
        Ok(self)
    }
}
