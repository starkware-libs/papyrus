...
fn get_db_version(db_config: &DbConfig) -> StorageResult<Option<Version>> {
...
fn to_v0(db_config: &DbConfig) -> StorageResult<()> {
    let (_, mut db_writer) = open_env(db_config.clone())?;
    let version_table_id: TableIdentifier<String, Version> =
        db_writer.create_table("storage_version")?;
...
#[instrument(skip(db_config), level = "debug", err)]
pub(crate) fn migrate_db(db_config: &DbConfig) -> StorageResult<()> {
    debug!("Beginning storage migration.");
    loop {
        let db_version = get_db_version(db_config)?;
        debug!(version=?db_version, "Upgrading version.");
        match db_version {
            // Upgrade storage version.
            None => to_v0(db_config)?,
