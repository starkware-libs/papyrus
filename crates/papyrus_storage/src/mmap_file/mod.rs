...
pub struct FileReader<'a> {
    shared_data: &'a [std::sync::atomic::AtomicU8],
}

impl<V: StorageSerde> Reader<V> for FileReader<'_> {
    /// Returns an object from the file.
    fn get(&self, location: LocationInFile) -> V {
        let bytes = std::borrow::Cow::from(unsafe {
            let as_u8: &[u8] = std::mem::transmute::<_, &[u8]>(self.shared_data);
            &as_u8[location.offset..location.offset + location.len]
        });
...
pub(crate) fn open_mmaped_file<V: StorageSerde>(path: PathBuf) -> MMapFile<V> {
...
/// Open a mmaped file, create it if it doesn't exist.
pub(crate) fn open_file<V: StorageSerde>(path: PathBuf) -> (FileWriter<V>, FileReader<'static>) {
    let file = OpenOptions::new().read(true).write(true).create(true).open(path).unwrap();
    let size = file.metadata().unwrap().len();
    let mmap =
        unsafe { MmapOptions::new().len(MAX_SIZE.try_into().unwrap()).map_mut(&file).unwrap() };
    let l_file =
        MMapFile { file, mmap, size: size.try_into().unwrap(), _value_type: PhantomData {} };
    let reader = FileReader { shared_data: unsafe { std::mem::transmute(l_file.mmap.as_ref()) } };
