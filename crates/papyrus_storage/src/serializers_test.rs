...
fn hints_regression() {
    ...
    .unwrap();
    let mut encoded_hints: Vec<u8> = Vec::new();
    casm.hints.serialize_into(&mut encoded_hints).expect("Failed to serialize hints");
    insta::assert_yaml_snapshot!(encoded_hints);
}
