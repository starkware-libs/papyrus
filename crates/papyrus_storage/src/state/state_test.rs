...
fn test_append_diff() -> Result<(), anyhow::Error> {
    ...
    txn = txn.append_state_diff(BlockNumber::new(0), diff0.clone())?;
    ...
    assert_eq!(txn.get_state_diff(BlockNumber::new(1))?, None);
    txn = txn.append_state_diff(BlockNumber::new(1), diff1.clone())?;
    ...
    if let Err(err) = txn.append_state_diff(BlockNumber::new(2), diff1) {
    ...
    if let Err(err) = txn.append_state_diff(BlockNumber::new(2), diff0) {
