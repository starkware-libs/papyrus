...
#[test]
fn revert_state() {
    let state0 = get_test_state_diff();
    let (contract0, class0) = state0.deployed_contracts.first().unwrap();
    let (_contract0, nonce0) = state0.nonces.first().unwrap();
    ...
    let nonce1 = Nonce(StarkFelt::from(0));
    let state1 = StateDiff {
    ...
    writer
        .begin_rw_txn()
        .unwrap()
        .append_state_diff(BlockNumber(0), state0.clone(), vec![])
        .unwrap()
        .append_state_diff(BlockNumber(1), state1, vec![])
        .unwrap()
        .commit()
        .unwrap();
    ...
    assert_matches!(deleted_data, Some((_thin_state_diff, _class_definitions)));
