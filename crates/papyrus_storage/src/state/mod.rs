...
fn write_storage_diffs<'env>(
    ...
    for (address, storage_entries) in state_diff.storage_diffs() {
...
fn delete_declared_classes<'env>(
    ...
    let deployed_contracts_class_hashes =
        thin_state_diff.deployed_contracts().iter().map(|(_, class_hash)| class_hash);
...
fn delete_deployed_contracts<'env>(
    ...
    for contract_address in thin_state_diff.deployed_contracts().iter().map(|(address, _)| address)
    {
...
fn delete_nonces<'env>(
    ...
    for (contract_address, _) in thin_state_diff.nonces() {
