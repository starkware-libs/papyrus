    ...
    Self: Sized,
{
    ...
    /// *Arg deployed_contract_class_definitions: Until Starknet version 0.11 users could implicitly
    /// declare new classes by deploying contracts. To append a state diff with such classes, you
    /// must pass the class definitions of the implicitly declared classes. To enforce that no
    /// commit happen after a failure, we consume and return Self on success.
