...
impl<'env, Mode: TransactionKind> StateReader<'env, Mode> {
    ...
    fn new(txn: &'env StorageTxn<'env, Mode>) -> StorageResult<Self> {
        ...
        Ok(StateReader {
            ...
            nonces_table,
            storage_table,
            file_access: txn.file_access.clone(),
        })
    }
    ...
    pub fn get_class_definition_at(
        ...
        Ok(Some(self.file_access.get_contract_class_unchecked(contract_class_location)?))
    }
...
fn delete_compiled_classes<'a, 'env>(
    ...
    for class_hash in class_hashes {
        ...
        deleted_data.insert(*class_hash, file_access.get_casm_unchecked(compiled_class_location)?);
    }
