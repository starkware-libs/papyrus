...
impl<'env, Mode: TransactionKind> StateReader<'env, Mode> {
    /// Create a new state reader from a storage transaction.
    ///
    ...
    /// Returns the storage at a given state number for a given contract and key.
    ...
    /// Returns the block number for a given class hash (the block it was defined in).
