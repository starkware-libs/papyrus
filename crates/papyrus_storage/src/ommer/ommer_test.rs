...
fn extract_body_data_from_storage(
    ...
    let transactions = txn.get_block_transactions(block_number)?.unwrap();
    let thin_transaction_outputs = txn.get_block_transaction_outputs(block_number)?.unwrap();
    ...
    let transaction_outputs_events = tx_indices
        .map(|tx_idx| txn.get_transaction_events(tx_idx).map(|e| e.unwrap()))
...
fn extract_state_diff_data_from_storage(
    ...
    let thin_state_diff = txn.get_state_diff(block_number)?.unwrap();
    ...
    let declared_classes: Vec<DeclaredContract> = class_hashes
        ...
        .map(|class_hash| {
            state_reader
                ...
                .map(|c| DeclaredContract { class_hash: *class_hash, contract_class: c.unwrap() })
        })
        .collect::<Result<_, _>>()?;
