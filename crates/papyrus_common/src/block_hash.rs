...
/// Validate hash of a starknet block.
...
pub fn calculate_block_commitments(
    ...
    let v3_transactions_commitment = get_transactions_commitment(block_body, true)?;
    ...
    let deprecated_transactions_commitment = get_transactions_commitment(block_body, false)?;
...
// Return the Patricia root of the transactions.
fn get_transactions_commitment(
    block_body: &BlockBody,
    is_version_3_and_up: bool,
) -> Result<StarkFelt, StarknetApiError> {
    let transaction_patricia_leaves =
        zip(block_body.transactions.iter(), block_body.transaction_hashes.iter())
            .map(|(transaction, transaction_hash)| {
                get_transaction_leaf(transaction, transaction_hash, is_version_3_and_up)
            })
            .collect::<Result<Vec<_>, _>>()?;
    Ok(calculate_root(transaction_patricia_leaves))
}

// Return a Patricia leaf value for a transaction.
fn get_transaction_leaf(
    transaction: &Transaction,
    transaction_hash: &TransactionHash,
    is_version_3_and_up: bool,
) -> Result<StarkHash, StarknetApiError> {
    let signature = if is_version_3_and_up {
...
// Return the Patricia root of the events.
