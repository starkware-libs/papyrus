...
fn validate_block_hash_by_version(
    ...
    let transaction_patricia_leaves =
        zip(block.body.transactions.iter(), block.body.transaction_hashes.iter())
    ...
    // Events leaves.
    let (n_events, events_patricia_root) = if version >= BlockHashVersion::V1 {
        let event_patricia_leaves: Vec<_> = block
            .body
            .transaction_outputs
            .iter()
    ...
    let block_hash = PedersenHashChain::new()
        ...
        .chain(&calculate_root(transaction_patricia_leaves))
        .chain(&n_events)
...
fn get_transaction_leaf(
    ...
    let signature = if version >= &BlockHashVersion::V3 {
        ...
        get_invoke_signature(transaction)
    };
...
fn get_invoke_signature(transaction: &Transaction) -> Vec<StarkFelt> {
