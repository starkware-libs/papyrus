...
lazy_static! {
    static ref SIERRA_API_VERSION: Felt = Felt::from_bytes_be_slice(b"CONTRACT_CLASS_V0.1.0");
}

const DEPRECATED_CLASS_API_VERSION: Felt = Felt::ZERO;
...
pub fn calculate_deprecated_class_hash(
    ...
    Ok(ClassHash(Pedersen::hash_array(&[
...
fn builtins_hash(
...
) -> Result<Felt, DeprecatedClassHashCalculationError> {
    ...
    // strings to bytes and from bytes to Felt.
    // TODO(yair): Consider deserializing to a builtin enum.
    let builtins_as_felts = builtins
        .iter()
        .map(|builtin_json| -> Result<Felt, DeprecatedClassHashCalculationError> {
            let builtin_as_bytes = builtin_json
                ...
                .as_bytes();
            Ok(Felt::from_bytes_be_slice(builtin_as_bytes))
        })
        .collect::<Result<Vec<_>, _>>()?;
    Ok(Pedersen::hash_array(builtins_as_felts.as_slice()))
}
...
fn bytecode_hash(
    ...
    let bytecode_as_felts = bytecode
        ...
        .map(|j| {
            ...
            let Ok(felt) = Felt::from_hex(as_str) else {
                return Err(DeprecatedClassHashCalculationError::BadProgramJson(
        ...
        .collect::<Result<Vec<_>, _>>()?;
    Ok(Pedersen::hash_array(bytecode_as_felts.as_slice()))
}
