...
fn new_urls() {
    ...
    let starknet_client = StarknetClient::get_test_client(url_base_str, get_test_config()).unwrap();
    assert_eq!(
...
async fn get_block_number() {
    let starknet_client =
        StarknetClient::get_test_client(&mockito::server_url(), get_test_config()).unwrap();
