...
use crate::{ClientCreationError, ClientError, StarknetClient, StarknetError, StarknetErrorCode};

/// Errors that may be returned from a reader client.
#[derive(thiserror::Error, Debug)]
pub enum ReaderClientError {
    /// A client error coming from the base client struct.
    #[error(transparent)]
...
impl StarknetFeederGatewayClient {
    ...
    async fn request_with_retry_url(&self, url: Url) -> ReaderClientResult<String> {
        self.client
            ...
            .await
            .map_err(ReaderClientError::ClientError)
    }
...
impl From<serde_json::Error> for ReaderClientError {
    fn from(error: serde_json::Error) -> Self {
        Self::ClientError(ClientError::SerdeError(error))
    }
}
