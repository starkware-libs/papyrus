    ...
    pub async fn class_by_hash(&self, class_hash: ClassHash) -> Result<Vec<u8>, ClientError> {
        ...
        match response {
            Ok(raw_contract_class) => {
                // To make sure raw_contract_class corresponds to a valid ContractClass, we
                // deserialize to ContractClass and then to bytes, instead of converting it to bytes
                // directly.
