...
impl StarknetClient {
    ...
    fn should_retry(err: &ClientError) -> bool {
        matches!(
            err,
            ClientError::BadResponse { status: StatusCode::REQUEST_TIMEOUT }
                | ClientError::BadResponse { status: StatusCode::TOO_MANY_REQUESTS }
                | ClientError::BadResponse { status: StatusCode::SERVICE_UNAVAILABLE }
                | ClientError::BadResponse { status: StatusCode::GATEWAY_TIMEOUT }
        )
    }
