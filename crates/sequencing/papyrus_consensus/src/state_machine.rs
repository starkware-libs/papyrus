...
pub enum StateMachineEvent {
    ...
    /// 1. If a block hash was supplied by the state machine, the caller responds with the supplied
    ///    block hash.
    /// 2. If no block hash was supplied by the state machine, the caller must find/build a block
    ///    to respond with.
