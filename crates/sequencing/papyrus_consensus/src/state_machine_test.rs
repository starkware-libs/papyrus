...
use crate::test_utils::validator_id;

fn create_state_machine(is_proposer: bool) -> StateMachine {
    StateMachine::new(
        if is_proposer { validator_id(1) } else { validator_id(2) },
        vec![validator_id(1), validator_id(2), validator_id(3), validator_id(4)],
        Box::new(|_| validator_id(1)),
    )
}
