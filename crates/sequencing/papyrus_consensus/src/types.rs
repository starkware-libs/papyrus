//! This module's primary content are:
//! 1. ConsensusBlock - a trait used to allow consensus to remain indifferent to the specific
//!    content of a block.
//! 2. Context - the primary interface for consensus to call out to the node.
#[cfg(test)]
#[path = "types_test.rs"]
mod types_test;
...
//
// Design Doc - https://docs.google.com/document/d/1ZJtgEcPzNW3hmzEIXNh-czY_OIEavNmXMj_4CeLA4A8/edit#heading=h.8csvpdbnvw8m
pub trait ConsensusBlock {
    ...
    type ProposalIter: Iterator<Item = Self::StreamT>;
    ...
    /// out only the information used to build the proposal, not the built information (ie the
    /// transactions, not the state diff).
...
//
// Design Doc - https://docs.google.com/document/d/1ZJtgEcPzNW3hmzEIXNh-czY_OIEavNmXMj_4CeLA4A8/edit#heading=h.vba42y9rmbmx
#[async_trait]
pub trait Context: Send + Sync {
    ...
    ///   and building it. If the block fails to be built, the Sender will be dropped by Context.
    ...
    ///
    /// Returns:
    /// - A receiver for the fully built block once it has been validated. If the block fails to be
    ///   built, the Sender will be dropped by Context.
    ...
    /// on blocks.
    async fn validators(&self, height: u64) -> BTreeSet<NodeId>;

    /// Calculates the ID of the Proposer based on the inputs.
    fn proposer(&self, validators: &BTreeSet<NodeId>, height: u64) -> NodeId;
}
