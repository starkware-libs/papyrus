...
class Node:
    ...
    def check_height(self):
        ...
        if self.height_and_timestamp[0] != height:
            if self.height_and_timestamp[0] and height:
                assert int(height) > int(
                    self.height_and_timestamp[0]
                ), f"Height should be increasing."
...
def monitor_simulation(nodes, start_time, duration, stagnation_timeout):
    ...
    for node in nodes:
        ...
        # Check if the height hasn't updated within the stagnation timeout period.
        # Collect the IDs of the nodes that have stagnated.
