...
async fn state_update_with_empty_storage_diff() {
    ...
    // Change the roots schema to match what the deserializer expects. To deserialize the state root
    // it should be a hex string without a "0x" prefix but the serializer adds a "0x" prefix.
