...
use starknet_api::{
    BlockNumber, BlockStatus, ChainId, ClassHash, ContractAddress, EventIndexInTransactionOutput,
    GlobalRoot, Nonce, StarkFelt, StarkHash, StateNumber, StorageKey, TransactionHash,
    TransactionOffsetInBlock, GENESIS_HASH,
};
...
impl JsonRpcServer for JsonRpcServerImpl {
    ...
    fn get_events(
        ...
        let from_block_number = filter
            .from_block
            .map_or(Ok(BlockNumber::new(0)), |block_id| get_block_number(&txn, block_id))?;
            ...
            .map_err(internal_server_error)?
        {
            ...
            if filter.keys.iter().enumerate().all(|(i, keys)| {
                ...
                let emitted_event = Event {
                    ...
                    event: starknet_api::Event { from_address, content },
                };
