...
/// there can ve multiple versions that are supported (latest is implicitly supported), and there can be multiple versions that are deprecated.
...
pub enum VersionState {
    // TODO: nevo - remove the dead_code attribute once other versions are implemented - hides "Supported" and "Deprecated" not constructed error
    Latest,
    #[allow(dead_code)]
    Supported,
    #[allow(dead_code)]
