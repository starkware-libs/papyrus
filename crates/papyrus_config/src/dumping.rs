...
pub fn ser_required_param(
    ...
    common_ser_param(
        name,
        SerializedContent::RequiredType(serialization_type),
        description,
        privacy.into(),
    )
}

/// Serializes expected type for a single param of a config that the system may generate. The random
/// generation should be defined in serde default field attribute.
/// The returned pair is designed to be an input to a dumped config map.
pub fn ser_random_param(
    name: &str,
    serialization_type: SerializationType,
    description: &str,
    privacy: ParamPrivacyInput,
    ...
    common_ser_param(
        name,
        SerializedContent::RandomOrCustomType(serialization_type),
        description,
        privacy.into(),
    )
}
