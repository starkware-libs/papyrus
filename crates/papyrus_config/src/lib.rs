...
pub enum SubConfigError {
    ...
    ParamNotFound,
    #[error("Unable to parsing {param_type} param type.")]
    NotImplementedParamType { param_type: String },
}
...
fn replace_json(prev_value: &Value, raw_new_data: &str) -> Result<Value, SubConfigError> {
    ...
    match prev_value {
        ...
        Value::Object(_) => {
            Err(SubConfigError::NotImplementedParamType { param_type: String::from("Object") })
        }
        Value::Array(_) => {
            Err(SubConfigError::NotImplementedParamType { param_type: String::from("Array") })
        }
    }
}

/// Returns a dumped configuration, updated with the new raw data.
pub fn update_dumped_config(
    mut dumped_config: BTreeMap<ParamPath, SerializedParam>,
    new_raw_data: BTreeMap<ParamPath, String>,
) -> Result<BTreeMap<ParamPath, SerializedParam>, SubConfigError> {
    for (param_path, val) in new_raw_data {
        let Some(ser_param) = dumped_config.get_mut(&param_path) else {
            return Err(SubConfigError::ParamNotFound);
        };
