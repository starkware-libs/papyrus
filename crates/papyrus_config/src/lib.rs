...
pub fn dump_to_file<T: SerializeConfig>(
    config: &T,
    file_path: &str,
    pointers: Vec<(ParamPath, Vec<ParamPath>)>,
) {
    let dumped = combine_config_map_and_pointers(config.dump(), pointers);
...
/// Takes a config map and a vector of {target param and vector of params that will point to it}.
...
fn combine_config_map_and_pointers(
    mut config_map: BTreeMap<ParamPath, SerializedParam>,
    pointers: Vec<(ParamPath, Vec<ParamPath>)>,
) -> BTreeMap<ParamPath, StoredParam> {
    let mut stored_map = BTreeMap::default();
    for (target_param, pointing_params_vec) in pointers {
        let first_pointing_param = pointing_params_vec.first().unwrap();
        let target_value = config_map.get(first_pointing_param).unwrap();
        stored_map.insert(target_param.to_owned(), StoredParam::RawParam(target_value.to_owned()));

        for pointing_param in pointing_params_vec {
            config_map.remove(&pointing_param);
            stored_map.insert(pointing_param, StoredParam::PointerParam(target_param.to_owned()));
        }
    }

    stored_map
        .into_iter()
        .chain(config_map.into_iter().map(|(k, v)| (k, StoredParam::RawParam(v))))
        .collect()
}
