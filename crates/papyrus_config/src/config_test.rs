...
use crate::dumping::{
    ...
    get_config_representation,
    get_public_config_representation,
    ser_optional_param,
...
fn test_config_representation() {
    ...
    let representation = get_config_representation(&config).unwrap();
    let keys: Vec<_> = representation.as_object().unwrap().keys().collect();
    assert_eq!(keys, vec!["a", "b", "c"]);

    let public_representation = get_public_config_representation(&config).unwrap();
