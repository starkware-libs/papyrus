...
fn test_config_representation() {
    impl ConfigRepresentation for TypicalConfig {}

    let config = TypicalConfig { a: Duration::from_secs(1), b: "bbb".to_owned(), c: false };
    let representation = config.get_config_representation().unwrap();
    let keys: Vec<_> = representation.as_object().unwrap().keys().collect();
    assert_eq!(keys, vec!["a", "b", "c"]);

    let public_representation = config.get_public_config_representation().unwrap();
