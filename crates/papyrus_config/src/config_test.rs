...
fn test_update_dumped_config() {
    ...
    assert_eq!(1234, updated_config_map.get("a").unwrap().value.as_u64().unwrap());
    assert_eq!("/abc", updated_config_map.get("b").unwrap().value.as_str().unwrap());
    assert!(updated_config_map.get("c").unwrap().value.is_null());
