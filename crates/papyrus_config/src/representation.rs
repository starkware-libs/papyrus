//! Representation of a configuration, with hiding or exposing private parameters.
...
pub fn get_config_representation<T: Serialize + SerializeConfig>(
    config: &T,
    include_private_parameters: bool,
) -> Result<serde_json::Value, ConfigError> {
    let mut config_representation = serde_json::to_value(config)?;
    if include_private_parameters {
        return Ok(config_representation);
    }

    // Iterates over flatten param paths for removing non-public parameters from the nested config.
    // For example, for the param path 'a.b.c.d', perform config_representation[a][b][c].remove(d).
