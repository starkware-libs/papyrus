...
pub fn bytes_from_hex_str<const N: usize, const PREFIXED: bool>(
    hex_str: &str,
) -> Result<[u8; N], Deserialization> {
    let hex_str = if PREFIXED {
        hex_str
            .strip_prefix("0x")
            .ok_or(Deserialization::MissingPrefix { hex_str: hex_str.into() })?
    } else {
        hex_str
    };
    ...
    if hex_str.len() > 2 * N {
        ...
        return Err(Deserialization::BadInput { expected_byte_count: N, string_found: err_str });
    }
