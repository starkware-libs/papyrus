...
use super::{bytes_from_hex_str, hex_str_from_bytes, Deserialization, HexAsBytes};
...
fn bytes_from_hex_str_errors() {
    ...
    assert_matches!(err, Err(Deserialization::BadInput { expected_byte_count: 1, .. }));

    // Invalid hex char.
    let err = bytes_from_hex_str::<1, false>("1z");
    assert_matches!(
        err,
        Err(Deserialization::FromHexError(hex::FromHexError::InvalidHexCharacter {
            c: 'z',
            index: 1
        }))
    );
    ...
    assert_matches!(err, Err(Deserialization::MissingPrefix { .. }));
