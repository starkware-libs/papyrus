...
#[test]
fn storage_diff_sorted() -> Result<(), anyhow::Error> {
    let storage_key_0 = StorageKey::try_from(shash!("0x0"))?;
    let storage_key_1 = StorageKey::try_from(shash!("0x1"))?;
    ...
    let address = ContractAddress::try_from(shash!("0x0"))?;
    let storage_diff = StorageDiff::new(address, unsorted_storage_entries)?;
...
#[test]
fn storage_diff_unique() -> Result<(), anyhow::Error> {
    let address = ContractAddress::try_from(shash!("0x0"))?;
    let storage_key = StorageKey::try_from(shash!("0x0"))?;
...
#[test]
fn state_sorted() -> Result<(), anyhow::Error> {
    let hash0 = shash!("0x0");
    let hash1 = shash!("0x1");

    let dep_contract_0 = DeployedContract {
        address: ContractAddress::try_from(hash0)?,
        class_hash: ClassHash(hash0),
    };
    let dep_contract_1 = DeployedContract {
        address: ContractAddress::try_from(hash1)?,
        class_hash: ClassHash(hash1),
    };
    let storage_diff_0 = StorageDiff::new(ContractAddress::try_from(hash0)?, vec![])?;
    let storage_diff_1 = StorageDiff::new(ContractAddress::try_from(hash1)?, vec![])?;
    ...
    let nonce_0 =
        ContractNonce { contract_address: ContractAddress::try_from(hash0)?, nonce: Nonce(hash0) };
    let nonce_1 =
        ContractNonce { contract_address: ContractAddress::try_from(hash1)?, nonce: Nonce(hash1) };
...
fn state_unique() -> Result<(), anyhow::Error> {
    ...
    let storage_diff =
        StorageDiff { address: ContractAddress::try_from(hash0)?, storage_entries: vec![] };
