...
fn state_unique() -> Result<(), anyhow::Error> {
    ...
    let deployed_contracts = vec![dep_contract.clone()];
    let storage_diffs = vec![storage_diff.clone()];
    let declared_contracts = vec![dec_contract.clone()];
    let nonces = vec![nonce.clone()];

    let state_diff = StateDiff::new(
        deployed_contracts.clone(),
        storage_diffs.clone(),
        declared_contracts.clone(),
        nonces.clone(),
    )?;
