...
fn state_sorted() {
    ...
    let sorted_storage_diffs = IndexMap::from([
        ...
        (ContractAddress(patricia_key1), sorted_storage_entries),
    ]);

    sort_state_diff(&mut state_diff);
    assert_eq!(state_diff.deployed_contracts, sorted_deployed_contracts);
    assert_eq!(state_diff.declared_classes, sorted_declared_contracts);
    assert_eq!(state_diff.storage_diffs, sorted_storage_diffs);
    assert_eq!(state_diff.nonces, sorted_nonces);
}
