...
async fn stream_state_updates() {
    ...
    assert_eq!(
        vec![(contract_address1, class_hash2), (contract_address2, class_hash3)],
        deployed_contracts
    );
    assert_eq!(vec![(contract_address1, vec![storage_entry])], storage_diffs);
    assert_eq!(
        vec![
    ...
    assert_eq!(vec![(contract_address1, nonce1)], nonces);
