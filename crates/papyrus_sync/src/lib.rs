...
> GenericStateSync<TCentralSource, TBaseLayerSource>
{
    ...
    async fn sync_while_ok(&mut self) -> StateSyncResult {
        ...
        let check_if_sync_stopped_progress =
            check_if_sync_stopped_progress(self.reader.clone()).fuse();
        pin_mut!(
            block_stream,
            state_diff_stream,
            compiled_class_stream,
            base_layer_block_stream,
            check_if_sync_stopped_progress
        );
        ...
        loop {
            ...
            let sync_event = select! {
              ...
              res = check_if_sync_stopped_progress.next() => res,
              complete => break,
            }
...
// The sleep time between checks if the sync stopped progress.
...
fn check_if_sync_stopped_progress(
    reader: StorageReader,
) -> impl Stream<Item = Result<SyncEvent, StateSyncError>> {
    try_stream! {
