    ...
    pub async fn run(&mut self) -> StateSyncResult {
        ...
        tokio::spawn({
            periodically_update_syncing_state(
                ...
                self.shared_syncing_state.clone(),
                starting_block,
            )
        });
        loop {
...
// Writes the updated syncing state every `syncing_state_sleep_duration`.
async fn periodically_update_syncing_state<TCentralSource: CentralSourceTrait + Sync + Send>(
    reader: StorageReader,
    central_source: Arc<TCentralSource>,
    ...
    loop {
        if let Ok(syncing_state) =
            get_syncing_state(&reader, &central_source, &starting_block).await
        {
            *shared_syncing_state.write().await = syncing_state;
...
async fn get_syncing_state<TCentralSource: CentralSourceTrait + Sync + Send>(
    ...
    let highest_block = loop {
        ...
        // In case of revert after getting the highest_block_number, the actual highest block number
        // might be lower, and this hash will be None.
