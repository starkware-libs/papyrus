...
pub const TEST_CONFIG_PATH: &str = "test_config.json";

pub fn get_test_rpc_config() -> RpcConfig {
    RpcConfig {
        chain_id: ChainId("SN_GOERLI".to_string()),
        execution_config_file: ExecutionConfig {
            config_file_name: TEST_CONFIG_PATH.to_string(),
        },
...
pub(crate) fn get_test_rpc_server_and_storage_writer_from_params<T: JsonRpcServerImpl>(
    ...
    (
        T::new(
            config.chain_id,
            config.execution_config_file.try_into().unwrap(),
            storage_reader,
