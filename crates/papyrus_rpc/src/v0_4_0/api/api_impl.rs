...
impl JsonRpcV0_4Server for JsonRpcServerV0_4Impl {
    ...
    async fn get_events(&self, filter: EventFilter) -> RpcResult<EventsChunk> {
        ...
        let block_id_opt_to_block_number = |block_id_opt| match block_id_opt {
            Some(BlockId::Tag(Tag::Pending)) | None => Ok(latest_block_number.next()),
            Some(block_id) => get_block_number(&txn, block_id),
        };
        let from_block_number = block_id_opt_to_block_number(filter.from_block)?;
        let mut to_block_number = block_id_opt_to_block_number(filter.to_block)?;
        ...
        if include_pending_block {
            let latest_block_hash =
                get_block_header_by_number::<_, BlockHeader>(&txn, latest_block_number)?.block_hash;
            ...
            let (transaction_start, event_start) = if event_index.0.0 > latest_block_number {
                (event_index.0.1.0, event_index.1.0)
            } else {
                (0, 0)
            };
                ...
                for (event_offset, event) in receipt.events.iter().cloned().enumerate() {
                    ...
                    if !filter.keys.iter().enumerate().all(|(i, keys)| {
                        event.content.keys.len() > i
                            && (keys.is_empty() || keys.contains(&event.content.keys[i]))
                    }) {
                        continue;
                    }
                    if let Some(filter_address) = filter.address {
