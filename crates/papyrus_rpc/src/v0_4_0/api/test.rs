...
async fn get_transaction_receipt() {
    ...
    let (mut client_transaction, _) = generate_client_transaction_and_rpc_transaction(&mut rng);
    let mut client_transaction_receipt = ClientTransactionReceipt::get_test_instance(&mut rng);
    *client_transaction.transaction_hash_mut() = pending_transaction_hash;
    ...
    let output = Into::<TransactionOutput>::into(starknet_api_output);
    let expected_result =
        GeneralTransactionReceipt::PendingTransactionReceipt(PendingTransactionReceipt {
    ...
    let (json_response, result) = raw_call::<_, TransactionHash, PendingTransactionReceipt>(
        &module,
        method_name,
        &Some(pending_transaction_hash),
    )
    .await;
    // See above for explanation why we compare the json strings.
    assert_eq!(
        serde_json::to_string(&result.clone().unwrap()).unwrap(),
