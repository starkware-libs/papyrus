...
async fn test_get_events(
    ...
    for (expected_event_indices, expected_continuation_token) in expected_result_by_index {
        ...
        let result = module.call::<_, EventsChunk>(method_name, [filter.clone()]).await.unwrap();
        let expected_events = expected_event_indices
            .iter()
        ...
        assert_eq!(
            result.continuation_token.clone().map(|x| x.parse().unwrap()),
...
async fn get_events_chunk_across_2_blocks() {
    let blocks_metadata = vec![
        BlockMetadata(vec![[DEFAULT_EVENT_METADATA; 4].to_vec(), vec![EventMetadata::default()]]),
