...
use papyrus_storage::{get_storage_version, StorageError, StorageReader, StorageVersion};
...
impl JsonRpcV0_4Server for JsonRpcServerV0_4Impl {
    ...
    async fn get_block_transaction_count(&self, block_id: BlockId) -> RpcResult<usize> {
        let Some(StorageVersion::FullArchive(_)) =
            get_storage_version(&self.storage_reader).map_err(internal_server_error)?
        else {
            return Err(internal_server_error_with_msg(StorageError::StateOnlyInvalidRequest));
        };
