...
use papyrus_common::python_json_formatter::PythonJsonFormatter;
...
fn calculate_deprecated_class_abi_length(
    deprecated_class: &starknet_api::deprecated_contract_class::ContractClass,
) -> Result<usize, ErrorObjectOwned> {
    let mut chars = vec![];
    deprecated_class
        .abi
        .serialize(&mut serde_json::Serializer::with_formatter(&mut chars, PythonJsonFormatter))
        .map_err(internal_server_error)?;
    Ok(chars.len())
}
...
fn get_class_lengths(
    ...
    state_number: StateNumber,
    class_hash: ClassHash,
) -> Result<(usize, usize), ErrorObjectOwned> {
    let state_number_after_block = StateNumber::right_after_block(state_number.block_after());
    storage_txn
        .get_state_reader()
        .map_err(internal_server_error)?
