...
fn test_append_diff() -> Result<(), anyhow::Error> {
    ...
    let diff0 = StateDiff {
        ...
        nonces: vec![(c0, Nonce(StarkHash::from_u64(0))), (c1, Nonce(StarkHash::from_u64(0)))],
    };
    ...
    assert_eq!(statetxn.get_nonce_at(state1, &c0)?, Some(Nonce(StarkHash::from_u64(0))));
    assert_eq!(statetxn.get_nonce_at(state2, &c0)?, Some(Nonce(StarkHash::from_u64(1))));
    ...
    assert_eq!(statetxn.get_nonce_at(state1, &c1)?, Some(Nonce(StarkHash::from_u64(0))));
    ...
    assert_eq!(statetxn.get_nonce_at(state2, &c2)?, Some(Nonce(StarkHash::from_u64(0))));
