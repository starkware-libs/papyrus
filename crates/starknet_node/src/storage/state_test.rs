...
use super::{StateStorageReader, StateStorageWriter, StorageError, ThinStatediff};
...
fn test_append_diff() -> Result<(), anyhow::Error> {
    ...
    let (thin_state_diff_0, _declared_classes_0) =
        ThinStatediff::full_diff_to_partial_and_classes(diff0);
    assert_eq!(txn.get_state_diff(BlockNumber(0))?.unwrap(), thin_state_diff_0);
    assert_eq!(txn.get_state_diff(BlockNumber(1))?, None);
    txn = txn.append_state_diff(BlockNumber(1), diff1.clone())?;
    let (this_state_diff_1, _declared_classes_1) =
        ThinStatediff::full_diff_to_partial_and_classes(diff1.clone());
    txn.commit()?;
