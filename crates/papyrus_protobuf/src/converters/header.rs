...
impl TryFrom<protobuf::SignedBlockHeader> for SignedBlockHeader {
    ...
    fn try_from(value: protobuf::SignedBlockHeader) -> Result<Self, Self::Error> {
        ...
        let state_diff_commitment = value.state_diff_commitment.and_then(|state_diff_commitment| {
            state_diff_commitment.root.and_then(|root| {
                StarkHash::try_from(root).ok().map(|felt| StateDiffCommitment(PoseidonHash(felt)))
            })
        });
