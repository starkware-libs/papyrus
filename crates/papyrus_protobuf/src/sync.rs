...
impl From<ThinStateDiff> for StateDiffChunkVec {
    fn from(mut thin_state_diff: ThinStateDiff) -> Self {
        // let mut thin_state_diff = thin_state_diff.clone();
        let mut state_diff_chunks = Vec::new();

        for (contract_address, class_hash) in thin_state_diff.deployed_contracts {
        ...
        for (contract_address, class_hash) in thin_state_diff.replaced_classes {
            state_diff_chunks.push(StateDiffChunk::ContractDiff(ContractDiff {
                contract_address,
                class_hash: Some(class_hash),
                nonce: thin_state_diff.nonces.get(&contract_address).cloned(),
        ...
        for class_hash in thin_state_diff.deprecated_declared_classes {
            state_diff_chunks.push(StateDiffChunk::DeprecatedDeclaredClass(
                DeprecatedDeclaredClass { class_hash },
            ));
        }
