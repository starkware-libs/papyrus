...
For example, to send a request calling the `starknet_getBlockTransactionCount` method, on block number 100000, using the 0.7.0 version of the API where `<node_path>` is `localhost:8080`, use the following command:
