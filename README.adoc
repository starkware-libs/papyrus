...
. Make a local directory to use for the container’s data, and set the correct permissions using the user and group IDs. You only need to complete this step the first time you run Papyrus.
+
[source,bash]
----
mkdir <local-host-data-path>
sudo chown -R $(id -u):$(id -g) /<local-host-data-path>
----
