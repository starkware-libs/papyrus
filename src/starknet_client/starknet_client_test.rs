...
async fn get_block_header() {
    ...
    let expected_block_header = BlockHeader {
        block_hash: BlockHash(StarkHash([0, 1, 2, 3])),
        parent_hash: BlockHash(StarkHash([0, 1, 2, 3])),
        gas_price: GasPrice(0x174876e800),
        number: BlockNumber(20),
        sequencer: ContractAddress(StarkHash([0, 1, 2, 3])),
        timestamp: BlockTimestamp(1636991716),
        state_root: GlobalRoot(StarkHash([0, 1, 2, 3])),
    };
