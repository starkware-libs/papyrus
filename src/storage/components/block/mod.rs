...
use crate::starknet::{
    ...
    StateDiffForward, StorageKey, Transaction, TransactionHash, TransactionIndex,
};
...
pub enum BlockStorageError {
    ...
    #[error(
        "Transaction hash {tx_hash:?} already exists, when adding transaction {tx_index:?} at block number {block_number:?}."
    )]
...
pub struct Tables {
    ...
    transaction_hash_to_idx: TableIdentifier<TransactionHash, (BlockNumber, TransactionIndex)>,
